...
const newLayout = `function Layout({ children, role, onLogout }) {
  ...
  const NavItem = ({ icon: Icon, label, menuId, colorClass, expanded, children }) => (
    <div>
      <button onClick={() => expanded && toggleMenu(menuId)} title={!expanded ? label : ""} className={\`w-full flex items-center justify-between py-3 rounded-xl font-bold transition-all hover:bg-white/5 hover:text-white \${activeMenu === menuId ? 'text-white bg-white/5' : ''} \${expanded ? 'px-4' : 'px-0 justify-center'}\`}>
         <div className="flex items-center gap-3">
           <Icon size={20} className={\`flex-shrink-0 \${colorClass}\`} />
           <span className={\`transition-all duration-300 whitespace-nowrap overflow-hidden \${expanded ? 'w-auto opacity-100' : 'w-0 opacity-0'}\`}>{label}</span>
         </div>
         {expanded && <ChevronDown size={14} className={\`transition-transform flex-shrink-0 \${activeMenu === menuId ? 'rotate-180' : ''}\`} />}
      </button>
      {expanded && activeMenu === menuId && (
  ...
  const renderSidebarContent = (expanded) => (
    <div className="flex flex-col h-full text-slate-300 overflow-x-hidden">
      <div className={\`flex items-center gap-3 py-6 border-b border-white/5 transition-all duration-300 \${expanded ? 'px-6' : 'px-4 justify-center'}\`}>
        <div className="w-10 h-10 flex-shrink-0 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center shadow-inner shadow-white/20">
          <Truck className="text-white w-6 h-6" />
        </div>
        <div className={\`transition-all duration-300 whitespace-nowrap overflow-hidden \${expanded ? 'w-auto opacity-100' : 'w-0 opacity-0 hidden'}\`}>
      ...
      <div className={\`flex-1 overflow-y-auto py-6 space-y-2 custom-scrollbar transition-all duration-300 \${expanded ? 'px-4' : 'px-3'}\`}>
        
        {/* Dashboard */}
        <NavLink onClick={closeMenu} to="/" title={!expanded ? "Dashboard" : ""} className={({isActive}) => \`flex items-center gap-3 py-3 rounded-xl font-bold transition-all \${isActive ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-white/5 hover:text-white'} \${expanded ? 'px-4' : 'px-0 justify-center'}\`}>
          <LayoutDashboard size={20} className={\`flex-shrink-0 \${location.pathname === '/' ? 'text-white' : 'text-indigo-400'}\`} />
          <span className={\`transition-all duration-300 whitespace-nowrap overflow-hidden \${expanded ? 'w-auto opacity-100' : 'w-0 opacity-0'}\`}>Dashboard</span>
        </NavLink>
        
        {/* Warehouse */}
        <NavItem icon={Package} label="Warehouse" menuId="warehouse" colorClass="text-emerald-400" expanded={expanded}>
        ...
        {/* Entry */}
        <NavItem icon={FileText} label="Operations" menuId="entry" colorClass="text-amber-400" expanded={expanded}>
        ...
        {/* Masters */}
        <NavItem icon={Users} label="Masters" menuId="masters" colorClass="text-blue-400" expanded={expanded}>
        ...
        {/* Print & Reports */}
        <NavItem icon={Printer} label="Print / Reports" menuId="reports" colorClass="text-purple-400" expanded={expanded}>
        ...
        {/* Settings */}
        <NavItem icon={Settings} label="Settings" menuId="settings" colorClass="text-slate-400" expanded={expanded}>
      ...
      <div className={\`p-4 border-t border-white/5 bg-black/20 flex items-center \${expanded ? 'justify-between' : 'justify-center flex-col gap-3'}\`}>
         <div className={\`transition-all duration-300 \${expanded ? 'w-auto opacity-100' : 'w-0 opacity-0 hidden'}\`}>
           <SystemStatus />
         </div>
  ...
  return (
    <div className="flex h-screen w-full bg-[#F3F4F6] selection:bg-indigo-200 overflow-hidden font-sans">
      ...
      <aside 
         ...
         className={\`hidden md:block h-full bg-[#0F172A] flex-shrink-0 z-50 shadow-2xl relative transition-all duration-300 ease-in-out \${isHovered ? 'w-[280px]' : 'w-24'}\`}
      >
...
console.log('Successfully updated App.jsx Layout to hover-expand');
